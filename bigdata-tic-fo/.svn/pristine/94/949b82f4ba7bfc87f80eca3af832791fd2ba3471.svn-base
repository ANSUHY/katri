...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		fn_searchMyRprtList('p', $("#currPage").val());
	...
	/*
	 * 함수명 : fn_searchMyRprtList(schType, p_page)
	 * 설	 명 : 검색할 경우(페이징x_페이징은 따로있음)
	 * @param schType : m(모바일), p(PC)
	*/
	function fn_searchMyRprtList(schType, p_page) {
		...
		/* ======== 3. data 가져오기 */
		fn_getMyRprtList(p_page);

	}

	/*
	 * 함수명 : fn_goPageMyRprtList(p_page)
	 * 설	 명 : 페이징(검색X_검색은 따로있음)
	*/
	function fn_goPageMyRprtList(p_page) {
	...
	*/
	function fn_getMyRprtList(p_page) {

		/* html비우기 */
		$("#areaMyRprt").html("");
		$("#areaMyRprtPaging").html("");

		/* 페이지 셋팅 */
		if ( fn_emptyCheck(p_page) || p_page == 0 ){
			p_page = 1;
		}
		$("#currPage").val(p_page);
	...
	function fn_afterGetMyRprtList(jsonData){
		...
		if(jsonData.resultCode === 200){
			...
			if(jsonData.data.list !== null && jsonData.data.list.length > 0){ /* 데이터가 있을 경우 */
				...
				// 2. 페이징 뿌리기
				fn_makePaging2("#areaMyRprtPaging", totCnt, currPage, rowCount, "fn_goPageMyRprtList");
...
<div id="container">
	...
	<div id="cont" class="cont-myservice board-myservice">
		...
		<form method="get" name="frm_myRprtList" 	id="frm_myRprtList">
			...
			<input type="hidden" name="currPage" 		id="currPage"			value="${schDatamyRprt.currPage == 0 ? 1 : schDatamyRprt.currPage}">
			...
			<div class="md-rect">
				...
				<div class="sch-wr">
					...
					<div class="btn-wr">
						...
						<a href="javascript:void(0);" onclick="fn_searchMyRprtList('m', '1')" class="btn md-r btn-test-sch">검색</a>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="mysvc-sch-wr" id="pSchInner">
			...
			<div class="btn-wr">
				...
				<a href="javascript:void(0);" onclick="fn_searchMyRprtList('p', '1')" class="btn b-sch">검색</a>
		...
		<div class="btn-wr paging">
			<div class="paging_wrap" id="areaMyRprtPaging">
			</div>
			<div class="btn_wrap">
