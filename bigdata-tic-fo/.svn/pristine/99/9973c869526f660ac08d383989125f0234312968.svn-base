...
<script type="text/javascript">
	...
	*/
	function fn_afterGetMyCertList(jsonData){
		...
		if(jsonData.resultCode === 200){
			...
			if(jsonData.data.list !== null && jsonData.data.list.length > 0){ /* 데이터가 있을 경우 */
				...
				// 1. 리스트 html 만들기
				// 1. 리스트 html 만들기
				html = fn_returnMyCertListHtml(jsonData.data.list, totCnt, currPage, rowCount);

				// 2. 페이징 뿌리기
				fn_makePaging2("#areaMyCertPaging", totCnt, currPage, rowCount, "fn_goPageMyCertList");
	...
	/*
	 * 함수명 : fn_returnMyCertListHtml(list, totCnt, currPage, rowCount)
	 * 설   명 : List html생성
	*/
	function fn_returnMyCertListHtml(list, totCnt, currPage, rowCount){
...
<div id="container">
	...
	<div id="cont" class="cont-myservice board-myservice">
		...
		<form method="get" name="frm_myCertList" 	id="frm_myCertList">
			...
			<input type="hidden" name="currPage" 		id="currPage"			value="${schDatamyCert.currPage == 0 ? 1 : schDatamyCert.currPage}"/>
		...
		<div class="mysvc-sch-wr" id="pSchInner">
			...
			<div class="btn-wr">
				<a href="javascript:void(0);" onclick="fn_resetSearch('p', '1')" class="btn reset">초기화</a>
				<a href="javascript:void(0);" onclick="fn_searchMyCertList('p', '1')" class="btn b-sch">검색</a>
