...
public class BoardController {
	...
	*****************************************************/
	 ...
	 * @throws Exception
	*****************************************************/
	@GetMapping(value = {"/board/{targetBoardNo}"})
	public ResponseEntity<ResponseDto> getBoardDetail( @PathVariable("targetBoardNo") Integer targetBoardNo, @ModelAttribute BoardSelectReq boardSelectReq) throws Exception {
		String msg 						= messageSource.getMessage("result-message.messages.common.message.error", null, SessionUtil.getLocale()); //에러가 발생하였습니다. 관리자에게 문의하여 주십시오.
		Integer resultCode 				= HttpStatus.BAD_REQUEST.value();
		...
		if(targetBoardNo == null || targetBoardNo == 0) {
			resultCode = HttpStatus.OK.value();
			msg = messageSource.getMessage("result-message.messages.common.message.wrong.appr", null, SessionUtil.getLocale()); // 잘못된 접근입니다.
		} else {

			try {
				// 데이터 조회
				boardDetail = boardService.getBoardDetail(targetBoardNo);
