...
<script type="text/javascript">
	...
	function fn_saveArchive() {
		...
		if (fn_validCheck()) {
			...
			// 데이터 폼
			let form = $("#archiveFrm")[0];
			let jData = new FormData(form);

			$.ajax({
				  url: "/ctnt/archive/saveArchive"
				, type: "POST"
	...
	function fn_getArchiveDetail() {
		...
		if (!fn_emptyCheck(opener.$("#targetNttSn").val())) {
			...
			$.ajax({
				  url: "/ctnt/archive/getArchiveDetail"
				, type: "GET"
				, dataType: "json"
					...
					if (jsonData.data !== null) {
						...
						if (data.fileDtoList !== null || data.fileDtoList.length !== 0) {
							...
							$.each(data.fileDtoList, function(index, data) {
								...
								fileHtml += "	<button type='button' class='btn btn-white btn-sm' onclick='fn_deleteFile(this, \"" + data.encodeFileSn + "\")'>- 삭제</button><br>";
								fileHtml += "<p>";
							});
...
<div id="pop-wrap">
	...
	<div class="pop-contnet">
		...
		<div class="btn-box">
			...
			<a href="javascript:void(0)" onclick="fn_saveArchive()" class="btn btn-blue">저장</a>
		</div>
