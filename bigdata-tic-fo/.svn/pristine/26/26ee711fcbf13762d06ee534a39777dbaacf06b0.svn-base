...
!function ($) {
	 ...
	 */
	bTabs.prototype.addTab = function(id,title,url,menuId,loginCheck){
		if(!id || !title || !url) console.error('Tab parameters error!');
		...
		$('ul.nav-tabs',$tabs).append('<li><a href="#'+tabId+'" menu_id=' + menuId + ' data-toggle="tab">'+title+c.closeBtnTemplate+'</a></li>');
		...
		var openIframe = function(){
			$(content).append('<iframe class="demo" frameborder="0" id="demo_'+tabId+'" title="'+title+'" scrolling="no" style="width:100%;height:100%;border:0px;" src="'+url+'"></iframe>');
			$('iframe.demo').on('load', function() {
				//fn_frameInit(this, menuId);
	        });
		};
		//进行登录验证
	 ...
	 * 新增标签页
	 */
	function bTabsAdd(id,title,url,menuId,loginCheck){
		return this.each(function(){
			if(!id || !title || !url) return;
			var $this = $(this),data = $this.data('bTabs');
			if(data) data.addTab(id,title,url,menuId,loginCheck);
		});
	}
	/**
	 * 关闭标签页
