...
(function(exports, module, define) {
  ...
  validate.validators = {
    ...
    email: v.extend(function(value, options) {
      ...
      // 이메일 체크 시 세미콜론(;) 포함할 수 있도록 수정 (2021.06.21)
      /*if (!this.PATTERN.exec(value)) {
        return message;
      }*/
		var ptrn = this.PATTERN;
		var chk = true;
		each(value.split(';'), function(v1) {
			if (v1 && !ptrn.exec(v1)) {
				chk = false;
				return false;
			}
 ...
 validate.extend(validate.validators.datetime, {
        ...
        format: function(value, options) {
          var format = options.dateOnly ? "DD-MM-YYYY" : "YYYY-MM-DD hh:mm:ss";
          return moment.utc(value).format(format);
 }
 });
 	var success = false;

 	var valid = function (formId, constraints){
    ...
    var handleFormSubmit = function(form, constraints) {
     ...
     var  showErrors = function(form, errors) {
        ...
        each(form.querySelectorAll("input[name], select[name], textarea[name]"), function(input) {
          // Since the errors can be null if no errors were found we need to handle
          // that

          var er = errors && errors[input.name];
          if(er && chk == 0){
            showErrorsForInput(input, er);
            chk = 1;
          }
        });
      }
