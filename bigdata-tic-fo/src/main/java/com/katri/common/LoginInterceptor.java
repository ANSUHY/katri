...
public class LoginInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (loginCheck(request, request.getSession(), serverClsCd)) {
			...
			//========== ## 1. 로그인 된 사람 DB에 있는 IP와 비교해서 기존 로그인 된사람 로그아웃 시키기(DEV로들어온거 제외 + 로그아웃페이지 제외)==========
			if( ("prd".equals(severSystemInfo)) && (!"/auth/logout".equals(request.getRequestURI())) //AAAAAAAAAAAAAAASH 성능테스트때문에 prd로 해놓음 dev로 바꿔야함
				...
				String strLoginUserId = SessionUtil.getLoginUserId(); //로그인된 아이디
				String strLoginRefVal = SessionUtil.getLoginRefVal(); //로그인시 로그인참조값

				// 1-2. 로그인 정보 조회
				LoginReq loginReq = new LoginReq();
				loginReq.setLoginId(strLoginUserId);
				...
				if(loginDetail != null) {
					...
					if( (! strNowIp.equals(loginDetail.getLastLgnIpAddr()) ) || (! strLoginRefVal.equals(loginDetail.getLgnRefVal()) ) ){ //안맞으면
						...
						//통신의 종류에 따라 처리
						if(StringUtils.isBlank(ajaxForward)) { //Ajax 통신이 아닐경우 - 로그인 페이지로 이동
			...
			//========== ## 2. 권한 체크 : ajax일때 안함 ==========
			if( StringUtils.isBlank(ajaxForward)
					&& Const.Session.SERVER_CLS_CD_FO.equals(serverClsCd) && !loginExcludePath(request.getRequestURI(), serverClsCd)) {
