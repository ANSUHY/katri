	...
	<div id="cont" class="cont-myservice board-myservice board-data">
			...
			<div class="md-rect">
				...
				<div class="sch-wr">
					<dl class="sch-lst">
						<dt>인증기관</dt>
						<dd>
							<select class="select" name="mSInstId" title="검색">
						...
						<dt>인증번호</dt>
						<dd>
							<input type="text" placeholder="입력하세요" name="mSCertNo" value="${schDatacertData.schCertNo}" title="검색"/>
						</dd>
						<dt>제조업체</dt>
						<dd>
							<input type="text" placeholder="입력하세요" name="mSMkrNm" value="${schDatacertData.schMkrNm}" title="검색"/>
						</dd>
						<dt>모델명</dt>
						<dd>
							<input type="text" placeholder="입력하세요" name="mSMdlNm" value="${schDatacertData.schMdlNm}" title="검색"/>
						</dd>
						<dt>수입업체</dt>
						<dd>
							<input type="text" placeholder="입력하세요" name="mSIptrNm" value="${schDatacertData.schIptrNm}" title="검색"/>
						</dd>
						<dt>사진유무</dt>
						<dd>
							<select class="select" name="mSPhotoYN" title="검색">
								<option value="">전체</option>
						...
						<dt>인증일자</dt>
						<dd>
							<input type="text" id="tblDatepicker01" placeholder="시작일"  readonly="readonly" name="mSCertYmdF" value="${schDatacertData.schCertYmdF}" title="검색"/>
							<input type="text" id="tblDatepicker02" placeholder="종료일"  readonly="readonly" name="mSCertYmdT" value="${schDatacertData.schCertYmdT}" title="검색"/>
						</dd>
						<dt>인증상태</dt>
						<dd>
							<select class="select" name="mSSttCd" title="검색">
								<option value="">전체</option>
						...
						<dt>제품분류</dt>
						<dd>
							<select class="select wd30" name="mSPrdtClfCd" id="mSPrdtClfCd" style="width: 100%;" title="검색">
								<option value="">전체</option>
						...
						<dt>제품명</dt>
						<dd>
							<input type="text" placeholder="입력하세요" name="mSPrdtNm" value="${schDatacertData.schPrdtNm}" title="검색"/>
						</dd>
					</dl>
					<div class="btn-wr">
		...
		<div class="mysvc-sch-wr" id="pSchInner">
			...
			<table class="tbl">
				<tr>
					<th>인증기관</th>
					<td>
						<select class="select" name="pSInstId" title="검색">
							<option value="">전체</option>
							<c:forEach items="${coInstList}" var="item">
								<option value="${item.lsInstId}" ${item.lsInstId eq schDatacertData.schInstId ? "selected" : ""} >${item.lsInstNm}</option>
							</c:forEach>
					...
					<th>인증번호</th>
					<td>
						<input type="text" placeholder="입력하세요" name="pSCertNo" class="searchInput" value="${schDatacertData.schCertNo}" title="검색"/>
					</td>
					<th>제조업체</th>
					<td>
						<input type="text" placeholder="입력하세요" name="pSMkrNm" class="searchInput" value="${schDatacertData.schMkrNm}" title="검색"/>
					</td>
				</tr>
				<tr>
					<th>모델명</th>
					<td>
						<input type="text" placeholder="입력하세요" name="pSMdlNm" class="searchInput" value="${schDatacertData.schMdlNm}" title="검색"/>
					</td>
					<th>수입업체</th>
					<td>
						<input type="text" placeholder="입력하세요" name="pSIptrNm" class="searchInput" value="${schDatacertData.schIptrNm}" title="검색"/>
					</td>
					<th>사진유무</th>
					<td>
						<select class="select" name="pSPhotoYN" title="검색">
							<option value="">전체</option>
							<option value="Y" ${ "Y" eq schDatacertData.schPhotoYN ? "selected" : ""}>유</option>
							<option value="N" ${ "N" eq schDatacertData.schPhotoYN ? "selected" : ""}>무</option>
						</select>
				...
				<tr>
					<th>인증일자</th>
					<td colspan="3">
						<input type="text" name="pSCertYmdF" id="tblDatepicker03" placeholder="시작일" readonly="readonly" value="${schDatacertData.schCertYmdF}" title="검색"/><span class="wd30">~</span><input type="text" name="pSCertYmdT" id="tblDatepicker04" placeholder="종료일" readonly="readonly" value="${schDatacertData.schCertYmdT}" title="검색"/>
					</td>
					<th>인증상태</th>
					<td>
						<select class="select" name="pSSttCd" title="검색">
							<option value="">전체</option>
							<c:forEach items="${certSttList}" var="item">
								<option value="${item.lsDtlCd}" ${item.lsDtlCd eq schDatacertData.schSttCd ? "selected" : ""} >${item.lsDtlCdNm}</option>
							</c:forEach>
				...
				<tr>
					<th>제품분류</th>
					<td colspan="3">
						<select class="select wd30" name="pSPrdtClfCd" id="pSPrdtClfCd" style="width: 100%;" title="검색">
							<option value="">전체</option>
							<c:forEach items="${prdtClfList}" var="item">
								<option value="${item.lsSttyPrdtClfCd}" ${item.lsSttyPrdtClfCd eq schDatacertData.schPrdtClfCd ? "selected" : ""} >${item.lsSttyPrdtDtclfNm}</option>
							</c:forEach>
					...
					<th>제품명</th>
					<td>
						<input type="text" placeholder="입력하세요" name="pSPrdtNm" class="searchInput" value="${schDatacertData.schPrdtNm}" title="검색"/>
					</td>
				</tr>
			</table>
			<div class="btn-wr">
		...
		<div class="board-cont" id="board-lst">
			<table class="data-lst">
				<caption>인증데이터 정보 제공 - 번호, 인증기관, 모델명, 제품명, 인증상태, 인증번호 내용 제공</caption>
				<thead>
					<tr>
						<th>번호</th>
						<th>인증기관</th>
