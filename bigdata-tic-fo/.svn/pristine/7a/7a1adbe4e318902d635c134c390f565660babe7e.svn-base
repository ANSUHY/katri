...
<c:set var="FILE_DIV_NM_GNRL_CERT_FILE" 		value="<%=FileConst.File.FILE_DIV_NM_GNRL_CERT_FILE%>" />
<c:set var="FILE_DIV_NM_BZMN_REG_FILE" 			value="<%=FileConst.File.FILE_DIV_NM_BZMN_REG_FILE%>" />
<c:set var="FILE_DIV_NM_ACNT_ISSU_APLY_FILE" 	value="<%=FileConst.File.FILE_DIV_NM_ACNT_ISSU_APLY_FILE%>" />

<spring:eval var="BRNO_API_KEY_ENCODING" expression="@environment.getProperty('brno.api.key.encoding')" />
...
<script type="text/javascript">
	...
	$(document).on("change", ".file_upload", function(){
		...
		if( $(this).val() != "" ) {
			...
			chkExt = ["png", "jpg", "gif", "pdf", "hwp", "doc", "docx"]; // 수정(2022-12-29)

			let chkSize = 1 * 1024 * 1024; // 1MB

			for(let i= 0; i<file.length; i++){

				// 파일 확장자
				targetExt = file[i].name.split('.').pop().toLowerCase();

				if(! ($.inArray(targetExt, chkExt) > -1)) {
					$(this).val("");
					$(this).parent().find(".file-ul").html("");
				...
				if( file[0].size > chkSize ) {
					$(this).val("");
					$(this).parent().find(".file-ul").html("");
					alert("<spring:message code='result-message.messages.join.message.attach.file.size.error'/>"); // '파일 크기가 맞지않습니다. 1MB 이하 파일만 첨부 가능합니다.'
					return false;
				}
			...
			// 파일 이름 html 출력
			let fileHtml = '<li><a href="javascript:void(0);" class="file-del">삭제</a>';
			fileHtml	+= file[0].name;
			fileHtml	+= '</li>';

			$(this).parent().find(".file-ul").html(fileHtml);
	...
	$(document).on("click", ".file-del", function(){
		...
		$(this).parent().parent().parent().find("input:file").val("");
		$(this).parent().remove();
	...
	// 그룹 추가 선택 시,
	$(document).on("click", '.typeGroup', function(event){

		let chkId = $(this).attr("id");

		if( chkId == "type-group-off" ) {
			$("#entGrpAddChkYn").val("N");
			$("#chkEntGrpNm").val("");
			$("#chkEntGrpMngNo").val("");
		} else {
			$("#entGrpAddChkYn").val("Y");
		}
	...
	*/
	function fn_joinInfoInit(){
		...
		// 사업자 정보 값 변경 시 - [사업자번호인증] 체크값 변경
		$("#chkEntNm").on("propertychange change paste input", function() {
			$("#brnoChkYn").val('N');
		});
		$("#tblDatepicker_chkOpbizYmd").on("propertychange change paste input", function() {
			$("#brnoChkYn").val('N');
		});
		$("#chkRprsvNm").on("propertychange change paste input", function() {
			$("#brnoChkYn").val('N');
		});
		$("#chkBrno").on("propertychange change paste input", function() {
			$("#brnoChkYn").val('N');
		});

		// 그룹명, 그룹ID 값 변경 시 - [그룹정보:중복확인] 체크값 변경
		$("#chkEntGrpNm").on("propertychange change paste input", function() {
			$("#entGrpDpcnChkYn").val('N');
		});
		$("#chkEntGrpMngNo").on("propertychange change paste input", function() {
			$("#entGrpDpcnChkYn").val('N');
		});
	...
	*/
	function fn_joinValidation() {
		...
		// 0. 사업자 정보 관련 검사
		if( ! fn_authBsnoValidation() ) {
			return false;
		}
		// 0_1. 사업자 등록번호 인증 검사
		if( "Y" != $("#brnoChkYn").val() ) {
			alert("<spring:message code='result-message.messages.join.message.brno.auth.required.checked'/>"); // '사업자 등록번호 인증을 진행해 주세요.'
			$("#chkBrno").focus();
			isValid = false;
			return false;
		}
		// 0_2. 사업자 등록번호 인증 후 셋팅 값 검사
		if( $("#chkBrno").val() != $("#brno").val() || $("#chkEntNm").val() != $("#entNm").val() ) {
			alert("<spring:message code='result-message.messages.join.message.brno.auth.required.checked'/>"); // '사업자 등록번호 인증을 진행해 주세요.'
			$("#chkBrno").focus();
			isValid = false;
			return false;
		}
		// 0_3. 사업자 등록번호 인증 후 셋팅 값 검사
		if( $("#tblDatepicker_chkOpbizYmd").val().replaceAll("-", "") != $("#opbizYmd").val() || $("#chkRprsvNm").val() != $("#rprsvNm").val() ) {
			alert("<spring:message code='result-message.messages.join.message.brno.auth.required.checked'/>"); // '사업자 등록번호 인증을 진행해 주세요.'
			$("#chkBrno").focus();
			isValid = false;
			return false;
		...
		// 1. 그룹 추가 - (예)
		if( $("#type-group-on").is(":checked") && "Y" == $("#entGrpAddChkYn").val() ) {
			// 1_0. 그룹명 검사
			if($.trim($("#chkEntGrpNm").val()) == "") {
				alert("<spring:message code='result-message.messages.common.message.required.data2' arguments='그룹명'/>"); // '{0}를(을) 입력해주세요.'
				$("#chkEntGrpNm").focus();
				isValid = false;
				return false;
			}
			// 1_1. 그룹 ID 검사
			if($.trim($("#chkEntGrpMngNo").val()) == "") {
				alert("<spring:message code='result-message.messages.common.message.required.data2' arguments='그룹ID'/>"); // '{0}를(을) 입력해주세요.'
				$("#chkEhkEntGrpMngNo").focus();
				isValid = false;
				return false;
			}
			// 1_2. 그룹명, 그룹ID 중복 검사
			if( "Y" != $("#entGrpDpcnChkYn").val() ) {
				alert("<spring:message code='result-message.messages.join.message.ent.grp.required.checked'/>"); // '그룹정보 중복 확인을 진행해 주세요.'
				$("#chkEntGrpNm").focus();
				isValid = false;
				return false;
			}
			// 1_3. 중복 확인 검사 후 셋팅 값 검사
			if( $("#chkEntGrpNm").val() != $("#entGrpNm").val() || $("#chkEntGrpMngNo").val() != $("#entGrpMngNo").val() ) {
				alert("<spring:message code='result-message.messages.join.message.ent.grp.required.checked'/>"); // '그룹정보 중복 확인을 진행해 주세요.'
				$("#chkEntGrpNm").focus();
				isValid = false;
				return false;
		...
		// 20. 관심 키워드1 중분류 중복 검사
		if( ! fn_keyWordValidation("select[name='arrStdMlclfCd1']") ) {
			return false;
		}

		// 21. 관심 키워드2 중분류 중복 검사
		if( ! fn_keyWordValidation("select[name='arrStdMlclfCd2']") ) {
			return false;
		}

		// 22. 첨부파일 검사 - 계정발급 신청서 첨부 검사
		if($.trim($("#acntIssuAplyFile").val()) == "") {
			alert("<spring:message code='result-message.messages.join.message.attach.file.required.data' arguments='계정발급 신청서'/>") // '{0}를(을) 첨부해 주세요.'
			$("#acntIssuAplyFile").click();
			isValid = false;
			return false;
		}

		// 23. 첨부파일 검사 - 사업자등록증 첨부 검사
		if($.trim($("#gnrlCertFile").val()) == "") {
			alert("<spring:message code='result-message.messages.join.message.attach.file.required.data' arguments='사업자 등록증'/>") // '{0}를(을) 첨부해 주세요.'
			$("#gnrlCertFile").click();
			isValid = false;
			return false;
		}

		// 24. 첨부파일 검사 - 재직증명서 첨부 검사
		if($.trim($("#bzmnRegFile").val()) == "") {
			alert("<spring:message code='result-message.messages.join.message.attach.file.required.data' arguments='재직증명서'/>") // '{0}를(을) 첨부해 주세요.'
			$("#bzmnRegFile").click();
			isValid = false;
			return false;
		}
	...
	*/
	...
	/*
	 * 함수명 : fn_authBsnoValidation()
	 * 설   명 : 사업자 등록번호 유효성 검사
	*/
	function fn_authBsnoValidation() {

		let isValid	= true;

		// 0. 사업자 번호 인증 - 상호명 미입력
		if($.trim($("#chkEntNm").val()) == "") {
			alert("<spring:message code='result-message.messages.common.message.required.data2' arguments='상호명'/>"); // '{0}를(을) 입력해주세요.'
			$("#chkEntNm").focus();
			isValid = false;
			return false;
		}

		// 1. 사업자 번호 인증 - 개업일자 미입력
		if($.trim($("#tblDatepicker_chkOpbizYmd").val()) == "") {
			alert("<spring:message code='result-message.messages.common.message.required.data2' arguments='개업일'/>"); // '{0}를(을) 입력해주세요.'
			$("#tblDatepicker_chkOpbizYmd").focus();
			isValid = false;
			return false;
		}

		// 2. 사업자 번호 인증 - 대표자명 미입력
		if($.trim($("#chkRprsvNm").val()) == "") {
			alert("<spring:message code='result-message.messages.common.message.required.data2' arguments='대표자명'/>"); // '{0}를(을) 입력해주세요.'
			$("#chkRprsvNm").focus();
			isValid = false;
			return false;
		}

		// 3. 사업자 번호 인증 - 사업자등록번호 미입력
		if($.trim($("#chkBrno").val()) == "") {
			alert("<spring:message code='result-message.messages.common.message.required.data2' arguments='사업자 등록번호'/>"); // '{0}를(을) 입력해주세요.'
			$("#chkBrno").focus();
			isValid = false;
			return false;
		}
		...
		// 4. 사업자 번호 글자수 확인
		if( $("#chkBrno").val().length > 10 ) {
			alert("<spring:message code='result-message.messages.join.message.brno.auth.word.cnt.error'/>"); // '10자를 초과 입력할 수 없습니다.'
			$("#chkBrno").focus();
			isValid = false;
			return false;
		}
	...
	*/
	function fn_authBsnoChk() {

		if( ! fn_authBsnoValidation() ) {
			return;
		}

		let entNm		= $("#chkEntNm").val();		// 상호명
		let opbizYmd 	= $("#tblDatepicker_chkOpbizYmd").val().replaceAll("-", "");	// 개업일자
		let rprsvNm 	= $("#chkRprsvNm").val();	// 대표자 성명
		let brno 		= $("#chkBrno").val(); 		// 사업자 등록 번호
		...
		let data = {
					"businesses":
						[
							{
							  "b_no"	: brno		// * 사업자 등록 번호 ('-' 제거)
							, "start_dt": opbizYmd	// * 개업일자 (YYYYMMDD 포맷)
							, "p_nm"	: rprsvNm	// * 대표자성명
							, "b_nm"	: entNm		// 상호 (Optional)
							}
						]
				};

		// 1. url + 인증키 셋팅
		let strUrl = "https://api.odcloud.kr/api/nts-businessman/v1/validate?serviceKey=${BRNO_API_KEY_ENCODING}";

		// 2. 사업자 번호 상태 조회 API 호출
		$.ajax({
			...
			success		: function(result) {
				...
				if( result.status_code === "OK" ) {
					...
					if( result.data[0].valid === "01" ) { // Valid
						...
						alert("<spring:message code='result-message.messages.join.message.brno.auth.success'/>"); // '사업장인증이 정상적으로 승인되었습니다.'

						$("#brnoChkYn").val('Y');
						$("#entNm").val(entNm);			// 상호명
						$("#opbizYmd").val(opbizYmd);	// 개업일자
						$("#rprsvNm").val(rprsvNm);		// 대표자 성명
						$("#brno").val(brno);			// 사업자 등록 번호

						// 3. 인증 확인 후, 해당 사업자 등록번호 조회 - 있는 경우 - 등록된 상호명 셋팅
						fn_brnoChkGetEntNm( brno );

					} else { // Invalid

						alert("<spring:message code='result-message.messages.join.message.brno.auth.error'/>"); // '유효하지 않은 사업자 등록번호입니다. 사업자 정보 확인 후, 다시 시도해 주세요.'
						$("#brnoChkYn").val('N');
						$("#entNm").val('');
						$("#opbizYmd").val('');
						$("#rprsvNm").val('');
	...
	/*
	 * 함수명 : fn_brnoChkGetEntNm()
	 * 설   명 : 인증 확인 후, 해당 사업자 등록번호 조회 - 상호명 셋팅
	 */
	function fn_brnoChkGetEntNm( brno ) {
			...
			, success 	: function(result) {
				...
				if (result.resultCode == "200") {
					...
					if ( result.data != null && result.data != "" ) {
						$("#chkEntNm").val(result.data);
						$("#entNm").val(result.data);
					}
	...
	/*
	 * 함수명 : fn_entGrpDpcnChk()
	 * 설   명 : 사업자등록번호 + 그룹ID + 그룹명 중복 체크
	 */
	function fn_entGrpDpcnChk() {
		...
		let brno 		= $("#chkBrno").val();
		let entGrpNm	= $("#chkEntGrpNm").val();
		let entGrpMngNo	= $("#chkEntGrpMngNo").val();

		$.ajax({
			url	 		: "/join/entGrpDpcnChk"
			, type 		: "POST"
			, dataType 	: "json"
			, data 		: {
							  "brno" 		: brno
							, "entGrpNm"	: entGrpNm
							, "entGrpMngNo" : entGrpMngNo
							}
			, success 	: function(result) {
				...
				if (result.resultCode == "200") {
					...
					if ( result.data ) {
						alert(result.resultMessage.replaceAll("\\n" , "\\\n").replaceAll("\\", ""));
						$("#entGrpDpcnChkYn").val('Y');
						$("#entGrpNm").val(entGrpNm);
						$("#entGrpMngNo").val(entGrpMngNo);
					} else {
						alert(result.resultMessage.replaceAll("\\n" , "\\\n").replaceAll("\\", ""));
						$("#entGrpDpcnChkYn").val('N');
						$("#entGrpNm").val('');
	 ...
	 * 설   명 : 해당 첨부파일 창 선택
	 */
	function fn_fileOpen(fileId) {
		$("#" + fileId).click();
	}

	/*
	 * 함수명 : fn_downloadAcntIssuFile()
	 * 설   명 : 파일 다운로드_DB에 없는거_[계정발급 신청서 양식 파일]
	*/
	function fn_downloadAcntIssuFile() {
		fn_pageFileDownload('test_hoho_v1.1.0.txt' , '기업 마스터 계정 발급 신청서(기업명).txt');
	}
	...
	<div id="container">
		...
		<div id="cont" class="cont-join join-insert">
			...
			<form name="frm_join" id="frm_join" method="post">
				...
				<input type="hidden" name="entNm" 			id="entNm" 			value="" />
				<input type="hidden" name="opbizYmd" 		id="opbizYmd" 		value="" />
				<input type="hidden" name="rprsvNm" 		id="rprsvNm" 		value="" />
				<input type="hidden" name="brno" 			id="brno" 			value="" />
				<input type="hidden" name="brnoChkYn" 		id="brnoChkYn" 		value="N" />

				<input type="hidden" name="entGrpAddChkYn"	id="entGrpAddChkYn" value="N" />
				<input type="hidden" name="entGrpNm"		id="entGrpNm"  		value=""  />
				<input type="hidden" name="entGrpMngNo"		id="entGrpMngNo" 	value=""  />
				<input type="hidden" name="entGrpDpcnChkYn" id="entGrpDpcnChkYn" value="N" />
				...
				<input type="text" readonly value="기업(마스터)회원" class="inp-readonly wd100" title="회원유형">

				<h4 class="tit"><span>소속 기업명</span></h4>
				<input type="text" id="chkEntNm" name="chkEntNm" value="" placeholder="사업자등록증의 상호명을 띄워쓰기 없이 입력해 주세요." class="wd100" maxlength="50" title="상호명"/>

				<h4 class="tit"><span>개업일</span></h4>
				<input type="text" id="tblDatepicker_chkOpbizYmd" name="chkOpbizYmd" value="" placeholder="사업자등록증의 개업일자 선택해 주세요." class="wd100 datepicker" readonly title="개업일"/>

				<h4 class="tit"><span>대표자명</span></h4>
				<input type="text" id="chkRprsvNm" name="chkRprsvNm" value="" placeholder="사업자등록증의 대표자 성명을 입력해 주세요." class="wd100"  maxlength="20" title="대표자명"/>

				<h4 class="tit"><span>사업자등록번호</span></h4>
				<div class="input-wr">
					<input type="text" id="chkBrno" name="chkBrno" value="" class="wd1287 number" placeholder="사업자등록번호 10자리를 입력해 주세요." maxlength="10" title="사업자 등록번호"/>
					<button type="button" class="btn bt-chk wd143" onclick="fn_authBsnoChk();">사업자번호 인증</button>
				</div>

				<h4 class="tit"><span>그룹 등록</span></h4>
				<div class="input-wr chk-r">
					<label for="type-group-off">
						<input type="radio" id="type-group-off" name="type-group" class="typeGroup" checked/>
						<span>아니요</span>
					</label>
					<label for="type-group-on">
						<input type="radio" id="type-group-on" name="type-group" class="typeGroup" />
						<span>예</span>
					</label>
					<p class="cau-txt">내 기업의 데이터 조회 권한이 그룹ID 기준으로 제한적으로 관리되기를 원하실 경우에만 '예' 선택 후 입력해 주세요.</p>
				</div>

				<div class="chk-r-wr">
					<h4 class="tit"><span>그룹명</span></h4>
					<div class="input-wr">
						<input type="text" id="chkEntGrpNm" name="chkEntGrpNm" class="wd1335" placeholder="그룹ID 쉬운 관리를 위해 별명을 입력하세요." maxlength="30" value="" title="그룹명"/>
					</div>
					<h4 class="tit"><span>그룹ID</span></h4>
					<div class="input-wr">
						<input type="text" id="chkEntGrpMngNo" name="chkEntGrpMngNo" class="wd1335" placeholder="시험인증 기관에 의뢰 시 등록된 기존 그룹ID(기관별 상이)를 동일하게 입력하세요." maxlength="20" value="" title="그룹ID"/>
						<button type="button" class="btn bt-chk" onclick="fn_entGrpDpcnChk();">중복확인</button>
					</div>
				</div>

				<h4 class="tit"><span>사업장 주소</span></h4>
				<div class="input-wr">
					<input type="text" id="entGrpZip" name="entGrpZip" value="" class="wd-zip" maxlength="10" title="사업장 주소"/>
					<button type="button" class="btn bt-chk wd-zip-btn" onclick="fn_openEntAddressPop();">우편번호 검색</button>
					</div>
				<div class="input-wr">
					<input type="text" id="entGrpBasAddr" 	name="entGrpBasAddr" value="" class="wd-addr1" readonly title="주소1"/>
					<input type="text" id="entGrpDaddr" 	name="entGrpDaddr"	 value="" class="wd-addr2" maxlength="30" title="주소2"/>
				</div>

				<h4 class="tit"><span>소속 부서명</span></h4>
				<input type="text" id="userDeptNm" name="userDeptNm" value="" placeholder="" class="wd100" maxlength="20" title="부서명"/>

				<h4 class="tit"><span>직위</span></h4>
				<input type="text" id="userJbgdNm" name="userJbgdNm" value="" placeholder="" class="wd100" maxlength="20" title="직급"/>

				<h4 class="tit"><span>직장 전화번호</span></h4>
				...
				<div class="input-wr">
					<input type="text" id="wrcBasAddr"	name="wrcBasAddr" 	value="" class="wd-addr1" readonly title="주소1"/>
				...
				<h4 class="tit"><span>성명</span></h4>
				<input type="text" class="wd100" id="userNm" name="userNm" maxlength="50" title="이름"/>

				<h4 class="tit"><span>휴대전화</span></h4>
				<input type="text" class="wd100 number" id="mblTelnoVal" name="mblTelnoVal" maxlength="20" title="휴대폰"/>

				<h4 class="tit"><span>직장 이메일주소</span></h4>
				<div class="input-wr">
					<input type="text" class="wd720" id="chkEmlAddrId" name="chkEmlAddrId" maxlength="30" title="이메일1"/><span class="wd25">@</span>
					<cm:makeTag cls="select" tagType="select" tagId="selEmlAddr" name="selEmlAddr" code="EML"/>
					<input type="text" class="inp-readonly wd285 ml10" id="chkEmlAddr" name="chkEmlAddr" maxlength="30" readonly title="이메일2"/>
				...
				<div class="info-rect tBr">
					<input type="file" id="acntIssuAplyFile" name="${FILE_DIV_NM_ACNT_ISSU_APLY_FILE}" class="file_upload" style="display: none;" />
					<button type="button" class="btn bt-chk wd-down-btn" onclick="fn_downloadAcntIssuFile();">양식 다운로드</button>
					<button type="button" class="btn bt-chk" onclick="fn_fileOpen('acntIssuAplyFile')">파일첨부</button>
					<p class="cau-txt file-txt">기업(마스터)회원 계정 발급을 위한 신청서 양식입니다.</p>
					<ul class="file-ul"></ul>
				</div>

				<h4 class="tit"><span>사업자등록증</span></h4>
				<div class="info-rect tBr">
					<button type="button" class="btn bt-chk" onclick="fn_fileOpen('gnrlCertFile')">파일첨부</button>
					<input type="file" id="gnrlCertFile" name="${FILE_DIV_NM_GNRL_CERT_FILE}" class="file_upload" style="display: none;" />
					<ul class="file-ul"></ul>
				</div>

				<h4 class="tit"><span>재직증명서</span></h4>
				<div class="info-rect tBr">
					<button type="button" class="btn bt-chk" onclick="fn_fileOpen('bzmnRegFile')">파일첨부</button>
					<input type="file" id="bzmnRegFile" name="${FILE_DIV_NM_BZMN_REG_FILE}" class="file_upload" style="display: none;" />
					<ul class="file-ul"></ul>
				</div>
