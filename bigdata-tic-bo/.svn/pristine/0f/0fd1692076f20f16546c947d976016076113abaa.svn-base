...
<c:set var="USER_STT_DRMNCY" 	value="<%=Const.Code.UserSttCd.DRMNCY%>" />
	...
	<script>
			...
			if( data.totCnt > 0 ) {
				...
				for( let nLoop=0; nLoop < data.list.length; nLoop++ ) {
					...
					userHtml += '<tr style="cursor:pointer;" onclick="fn_userBasDetailPop(this)" data-key="' + data.list[nLoop].userId + '" data-type="' + data.list[nLoop].userTyCd + '">';
						...
						// 휴면전환일
						userHtml += "	<td>" + data.list[nLoop].drmncyYmd + "</td>";
		...
		function fn_userBasDetailPop( obj ) {
			...
			if( targetUserId != '' ) {
				...
				$("#frm_drmncy #targetUserId").val(targetUserId);
				$("#frm_drmncy #targetUserTypeCd").val(targetUserTypeCd);

				frm_drmncy.action = "/user/user/userDetailPopUp";
				frm_drmncy.method = "POST";
				frm_drmncy.target = name;
				frm_drmncy.submit();
	...
	<div id="container">
		...
		<div class="titArea">
			<div class="location">
				<span class="ic-home"><i></i></span><span>회원 관리</span><em>휴면회원 현황</em>
			</div>
		</div>

		<!-- 검색 조건 -->
		<form action="" method="get" id="frm_drmncy" name="frm_drmncy">
			...
			<input type="hidden" name="searchUserSttCd" 	id="searchUserSttCd"	value="${USER_STT_DRMNCY}" />
			<input type="hidden" name="searchKeyword"		id="searchKeyword"		value="${USER_STT_DRMNCY}" />

			<div class="btn-box bot r">
				<div class="left">
					<h2 class="pop-tit2 left" style="margin-top: 0px;">휴면회원 현황</h2>
				</div>
