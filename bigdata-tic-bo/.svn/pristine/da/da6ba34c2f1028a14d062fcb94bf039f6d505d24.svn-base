    ...
    function html5Upload() {
    	...
    	sUploadURL= '/smarteditor/file_uploader'; 	//upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/smarteditor/file_uploader',
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	success : function(oCustomEvent) {
 	    		...
 	    		// }
 	    		var aResult = [];
 	    		oCustomEvent.htResult.sFileName = decodeURIComponent(decodeURI(oCustomEvent.htResult.sFileName).replace(/\+/g, " "));
 	    	...
 	    	error : function(oCustomEvent) {
 	    		...
 	    		alert(decodeURIComponent(decodeURI(oCustomEvent.htResult.errstr).replace(/\+/g, " ")));
