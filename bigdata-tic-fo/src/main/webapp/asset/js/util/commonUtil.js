 ...
 * 페이징 처리 함수2
 *************************************************************************/
function fn_makePaging2(target, totalCount, currPage, rowCount, fnCallback) {
	let pagePerBlcok = 5;

	// 페이징 관련 디폴트 값 설정
	if ($(target).length <= 0) target = ".paging_wrap";
	if (currPage <= 0) currPage = 1;
	...
	if (currBlock > 1) {
		...
		pageHtml += "<div class='prev_btn_wrap' onclick='" + fnCallback + "(" + ((currBlock - 1) * pagePerBlcok) + "); return false;'>";
		pageHtml += "	<i class='prev_btn'></i>";
		pageHtml += "	<span>Pre</span>";
		pageHtml += "</div>";
	...
	if (endPage == 0) {
		pageHtml += "<li><a class='active' href='javascript:void(0);'>1</a></li>";
	} else {
		for (let i = startPage; i <= endPage; i++) {
			if (i == currPage) {
				pageHtml += "<li><a class='active' href='javascript:void(0);'>" + i + "</a></li>";
			} else {
				pageHtml += "<li><a href='javascript:void(0);' onclick='" + fnCallback + "(" + i + "); return false;'>" + i + "</a></li>";
			}
		}
	}
	...
	if (currBlock < totalBlcok) {
		...
		pageHtml += "<div class='next_btn_wrap' onclick='" + fnCallback + "(" + ((currBlock * pagePerBlcok) + 1) + "); return false;' >";
		pageHtml += "	<span>Next</span>";
		pageHtml += "	<i class='next_btn'></i>";
		pageHtml += "</div>";
